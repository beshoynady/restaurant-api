...
const Products = () => {
  ...
  const editProduct = async (e) => {
    ...
    if (productimg) {
      try {
        const response = await axios.put('http://localhost:8000/api/product/' + productid, {
      ...
      try {
        const response = await axios.put('http://localhost:8000/api/product/withoutimage/' + productid, {
                ...
                if (i < pagination & i >= pagination - 5) {
                  return (
                    <tr key={i}>
                      ...
                      <td>{i + 1}</td>
                      <td><img src={`http://localhost:8000/${p.image}`} style={{ "width": "60px", "height": "50px" }} /></td>
                      <td>{p.name}</td>
                      <td>{p.description}</td>
