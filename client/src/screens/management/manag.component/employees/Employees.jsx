...
const Employees = () => {
  ...
  const createEmployee = async (e) => {
    ...
    try {
      const newemployee = await axios.post('http://localhost:8000/api/user', { username, password, address, phone, email, isAdmin, role, salary })
  ...
  const updateEmployee = async (e) => {
    ...
    try {
      if(password){
      const update = await axios.put('http://localhost:8000/api/user/' + userid, { username, password, address, phone, email, isAdmin, role, salary })
      console.log(update)
      if(update){
        getemployees()
      }
  }else{
      const update = await axios.put('http://localhost:8000/api/user/' + userid, { username, address, phone, email, isAdmin, role, salary })
      console.log(update)
      if(update){
        getemployees()
    } 
  ...
  const deleteEmployee = async (e) => {
    ...
    try {
      console.log(userid)
      const deleted = await axios.delete(`http://localhost:8000/api/user/${userid}`)
      if(deleted){
      console.log(deleted)
      getemployees()
    }
